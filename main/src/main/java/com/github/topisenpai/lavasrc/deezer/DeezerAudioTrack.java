...
public class DeezerAudioTrack extends DelegatedAudioTrack {
    ...
    private URI getTrackMediaURI() throws IOException, URISyntaxException {
        ...
        if (json.get("data").index(0).get("errors").index(0).get("code").asLong(0) != 0) {
            throw DeezerException.wrapFromPrivateAPI("Failed to get session ID", json);
        }
        ...
        if (json.get("data").index(0).get("errors").index(0).get("code").asLong(0) != 0) {
            throw DeezerException.wrapFromPrivateAPI("Failed to get user token", json);
        }
        ...
        if (json.get("data").index(0).get("errors").index(0).get("code").asLong(0) != 0) {
            throw DeezerException.wrapFromPrivateAPI("Failed to get track token", json);
        }
        ...
        if (json.get("data").index(0).get("errors").index(0).get("code").asLong(0) != 0) {
            throw DeezerException.wrapFromPrivateAPI("Failed to get media URL", json);
        }
